...
def run(vars, values):
    ...
    for di in d:
        for var in di:
            editVar(var, di[var])
        # Run simulation
        print( ' '.join([var + ' ' + str(di[var]) for var in di]) )
        os.system('./coarseRun > log.run')
        t = get_end_time_str()
        os.system('cp -r ./coarse/' + t + ' ./coarse/save-' + '-'.join([var + '-' + str(di[var]) for var in di])  )
        # Get results
        res = get_perf()

        this_strs = list(di.keys()) + list(res.keys())
        this_values = ['{:.6e}'.format(k) for k in list(di.values()) + list(res.values())]
